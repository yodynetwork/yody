Gitian building
================
*Setup instructions for a Gitian build of Yody Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Yody
...
- [Building Yody Core](#building-yody-core)
...
Clone the git repositories for yody and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/yodynetwork/yody --recursive
git clone https://github.com/yodynetwork/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/yody/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the yody repository under 'Fetch and create inputs' to install sources which require
...
To build Yody Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the yody repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yody/.git/
...
For example:
```bash
URL=https://github.com/laanwj/yody.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit yody=${COMMIT} --url yody=${URL} ../yody/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yody=${COMMIT} --url yody=${URL} ../yody/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yody=${COMMIT} --url yody=${URL} ../yody/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-yody:
./bin/gbuild --commit yody=${COMMIT},cpp-eth-yody=${ETHCOMMIT} --url yody=${URL},cpp-eth-yody=${ETHURL} ../yody/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the yody git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../yody/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/yodynetwork/yody-detached-sigs.git

BTCPATH=/some/root/path/yody
SIGPATH=/some/root/path/yody-detached-sigs

./bin/gbuild --url yody=${BTCPATH},signature=${SIGPATH} ../yody/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yody-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yody-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/yody-osx-build.assert
```
...
[yodynetwork/gitian.sigs](https://github.com/yodynetwork/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@yody.org) and he will commit them.
