...
The smart contract interface in Yody still requires some technical knowledge. The GUI is not completed yet, so all smart contract interation must happen either using `qtum-cli` at the command line, or in the debug window of `qtum-qt`. 
...
(note, if using the debug window in the Yody Qt application, don't include `./qtum-cli` in the commands)
...
* Q: "I think I found a bug in Yody" A: Please report any bugs at https://github.com/yodynetwork/qtum/issues
...
* `-record-log-opcodes` - This will create a new log file in the Yody data directory (usually ~/.qtum) named vmExecLogs.json, where any EVM LOG opcode is logged along with topics and data that the contract requested be logged. 
...
Additional documents for the overall design and expected results of various operations is available at the ITD repository here: https://github.com/yodynetwork/qtum-itds
