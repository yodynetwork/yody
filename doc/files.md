...
3. All content of the data directory, except for `yody.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `yodyd` or `yody-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
`./`               | `banlist.json`        | Stores the addresses/subnets of banned nodes.
`./`               | `yody.conf`           | User-defined [configuration settings](bitcoin-conf.md) for `yodyd` or `yody-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `yodyd.pid`           | Stores the process ID (PID) of `yodyd` or `yody-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `yodyd` or `yody-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [yody.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`yody-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
