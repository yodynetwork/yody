...
void YodyDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool YodyDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule YodyDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t YodyDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t YodyDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t YodyDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t YodyDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool YodyDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void YodyDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void YodyDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void YodyDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data, chainstate)[0].execRes.output;
}

void YodyDGP::createParamsInstance(){
...
dev::Address YodyDGP::getAddressForBlock(unsigned int blockHeight){
...
void YodyDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void YodyDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void YodyDGP::parseStorageOneUint64(uint64_t& value){
...
void YodyDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule YodyDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
