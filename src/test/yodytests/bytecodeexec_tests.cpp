...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract){
    initState();
    YodyTransaction txEth = createYodyTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YodyTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
    
    std::vector<dev::Address> addrs = {createYodyAddress(txs[0].getHashWith(), txs[0].getNVout())};
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGasIntrinsic){
    initState();
    YodyTransaction txEth = createYodyTransaction(CODE[0], 0, dev::u256(100), dev::u256(1), HASHTX, dev::Address());
    std::vector<YodyTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGas){
    initState();
    YodyTransaction txEth = createYodyTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YodyTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGasIntrinsic_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        YodyTransaction txEth = createYodyTransaction(CODE[0], 0, gasLimit, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createYodyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGas_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        YodyTransaction txEth = createYodyTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createYodyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_many){
    ...
    for(size_t i = 0; i < 130; i++){
        YodyTransaction txEth = createYodyTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createYodyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer){
    initState();
    YodyTransaction txEthCreate = createYodyTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YodyTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    std::vector<dev::Address> addrs = {createYodyAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout())};
    YodyTransaction txEthCall = createYodyTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, addrs[0]);
    std::vector<YodyTransaction> txsCall(1, txEthCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGasIntrinsic_return_value){
    ...
    std::vector<YodyTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    dev::Address newAddress(createYodyAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    YodyTransaction txEthCall = createYodyTransaction(ParseHex("00"), 1300, dev::u256(1), dev::u256(1), HASHTX, newAddress);
    std::vector<YodyTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGas_return_value){
    initState();
    YodyTransaction txEthCreate = createYodyTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YodyTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    dev::Address newAddress(createYodyAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    YodyTransaction txEthCall = createYodyTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, newAddress);
    std::vector<YodyTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_many){
    ...
    std::vector<YodyTransaction> txsCall;
    std::vector<dev::Address> addrs;
    valtype codeCall(ParseHex("00"));
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        YodyTransaction txEthCall = createYodyTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hash, newAddressGen[i], i);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_OutOfGas_transfer_many_return_value){
    ...
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 130; i++){
        YodyTransaction txEth = createYodyTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createYodyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    ...
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        YodyTransaction txEthCall = createYodyTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hashCall, newAddressGen[i], i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
        ++hashCall;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_suicide){
    initState();
    std::vector<YodyTransaction> txsCreate;
    std::vector<dev::Address> newAddresses;
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 10; i++){
        valtype code = i == 0 ? CODE[0] : CODE[4];
        dev::u256 value = i == 0 ? dev::u256(0) : dev::u256(13);
        YodyTransaction txEthCreate = createYodyTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        txsCreate.push_back(txEthCreate);

        YodyTransaction txEthSendValue;
        if(i != 0){
            txEthSendValue = createYodyTransaction(valtype(), value, GASLIMIT, dev::u256(1), hash, createYodyAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()), i);
            txsCreate.push_back(txEthSendValue);
        }

        newAddresses.push_back(createYodyAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
        ++hash;
    }
    ...
    for(size_t i = 0; i < newAddresses.size() - 1; i++){
        YodyTransaction txEthCall = createYodyTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddresses[i + 1]);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_contract_create_contracts){
    initState();
    YodyTransaction txEthCreate = createYodyTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<YodyTransaction> txs(1, txEthCreate);
    executeBC(txs, *m_node.chainman);
    std::vector<YodyTransaction> txsCall;
    valtype codeCall(ParseHex("3f811b80"));
    dev::Address newAddress(createYodyAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
    std::vector<dev::Address> addrs;
    for(size_t i = 0; i < 20; i++){
        YodyTransaction txEthCall = createYodyTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddress, i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall, *m_node.chainman);
