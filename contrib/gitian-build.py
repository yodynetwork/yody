...
def setup():
    ...
    if not os.path.isdir('qtum-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/yodynetwork/qtum-detached-sigs.git'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('qtum'):
        subprocess.check_call(['git', 'clone', 'https://github.com/yodynetwork/qtum.git'])
...
def build():
    ...
    if args.linux:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'qtum='+args.commit+',qtum-evmone=master', '--url', 'qtum='+args.url, '../qtum/contrib/gitian-descriptors/gitian-linux.yml'])
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'qtum='+args.commit+',qtum-evmone=master', '--url', 'qtum='+args.url, '../qtum/contrib/gitian-descriptors/gitian-win.yml'])
    ...
    if args.macos:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'qtum='+args.commit+',qtum-evmone=master', '--url', 'qtum='+args.url, '../qtum/contrib/gitian-descriptors/gitian-osx.yml'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/yodynetwork/qtum', help='Specify the URL of the repository. Default is %(default)s')
