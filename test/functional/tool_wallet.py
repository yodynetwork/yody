...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def bitcoin_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/yody-wallet' + self.config["environment"]["EXEEXT"]
    ...
    def get_expected_info_output(self, name="", transactions=0, keypool=2, address=0):
        ...
        if self.options.descriptors:
            output_types = 4  # p2pkh, p2sh, segwit. pk
        else:
            output_types = 3
        if self.options.descriptors:
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        self.assert_raises_tool_error('No method provided. Run `yody-wallet -help` for valid methods.')
        self.assert_raises_tool_error('Wallet name must be provided when creating a new wallet.', 'create')
        locked_dir = os.path.join(self.options.tmpdir, "node0", "regtest", "wallets")
        error = 'Error initializing wallet database environment "{}"!'.format(locked_dir)
        if self.options.descriptors:
            error = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another yodyd?"
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of yody-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        ...
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of yody-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not os.path.isdir(os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload"))
