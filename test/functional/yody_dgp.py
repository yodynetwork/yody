...
class YodyDGPTest(BitcoinTestFramework):
    def set_test_params(self, split=False):
        self.setup_clean_chain = True
        self.num_nodes = 1
        self.extra_args = [['-txindex=1', '-logevents']]
    ...
    def run_test(self):
        ...
        # Fix for increased gas costs -> set the dgp block gas limit really high
        self.BLOCK_SCHEDULE_DGP = DGPState(self.node, "0000000000000000000000000000000000000080")
        # Start off by setting ourself as admin
        admin_address = self.node.getnewaddress()

        # Set ourself up as admin
        self.BLOCK_SCHEDULE_DGP.send_set_initial_admin(admin_address)
        self.node.generate(1)
        self.create_proposal_contract()
        self.BLOCK_SCHEDULE_DGP.send_add_address_proposal(self.proposal_address, 2, admin_address)
