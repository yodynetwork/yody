...
class GenerateBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        combo_address = convert_btc_bech32_address_to_yody('bcrt1qw508d6qejxtdg4y5r3zarvary0c5xw7kygt080')
        ...
        combo_address = convert_btc_address_to_yody('mkc9STceoCcjoXEXe6cm66iJbmjM6zR9B2')
        hash = node.generateblock('combo(' + combo_key + ')', [])['hash']
        block = node.getblock(hash, 2)
        assert_equal(len(block['tx']), 1)
        assert_equal(block['tx'][0]['vout'][0]['scriptPubKey']['address'], combo_address)
